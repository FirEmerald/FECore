...
public abstract class BoundingShapeUnbounded extends BoundingShape implements IUnbounded
{
	@Override
	...
	{
		return getEntityList(level, entity, filter).filter(e -> {
			Vec3 pos = e.position();
	...
	{
		return getEntityList(level, typeTest, filter).filter(e -> {
			Vec3 pos = e.position();
			return this.isWithin(e, pos.x, pos.y, pos.z, testerX, testerY, testerZ);
		});
	}
