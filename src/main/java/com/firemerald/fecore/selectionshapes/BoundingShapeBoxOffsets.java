...
public class BoundingShapeBoxOffsets extends BoundingShapeConfigurable
{
	...
	public int addPosition(Player player, BlockPos blockPos, int num)
	{
		if (num == 0)
		{
			...
			y = blockPos.getY() + .5;
			z = blockPos.getZ() + .5;
			player.sendMessage(new TranslatableComponent("fecore.shapetool.position.set", new Vec3d(x, y, z).toString()), Util.NIL_UUID);
	...
	public void addInformation(ItemStack stack, Level worldIn, List<Component> tooltip, TooltipFlag flagIn)
	{
		...
		tooltip.add(new TranslatableComponent("fecore.shapetool.tooltip.size", new Vec3d(sizeX, sizeY, sizeZ)));
	}
}