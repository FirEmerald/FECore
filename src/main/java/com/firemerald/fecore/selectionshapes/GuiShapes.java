...
@OnlyIn(Dist.CLIENT)
public class GuiShapes extends PopupScreen implements IShapeGui
{
    ...
    public static class ShapeToolButton extends Button
    {
    	...
    	public ShapeToolButton(int x, int y, Component buttonText, GuiShapes gui, Player player, InteractionHand hand, Runnable onClick)
    	{
    	...
    	public ShapeToolButton(int x, int y, int widthIn, int heightIn, Component buttonText, GuiShapes gui, Player player, InteractionHand hand, Runnable onClick)
    	{
    	...
    	public void tick()
    	{
    		...
    		if (!held.isEmpty())
    		{
    			...
    			this.enabled = cap.isPresent() && cap.resolve().get().canAcceptShape(gui.currentShape.shape);
    		}
	...
	public GuiShapes(Vec3 pos, BoundingShape shape, Consumer<BoundingShape> onAccept)
	{
		...
		if ((player = Minecraft.getInstance().player) != null)
		{
			...
			addRenderableWidget(fromToolMain = new ShapeToolButton(0, 0, 200, 20, new TranslatableComponent("fecore.shapesgui.mainhand.from"), this, player, InteractionHand.MAIN_HAND, () -> {
	    		...
	    		if (!held.isEmpty())
	    		{
	    			...
	    			if (cap.isPresent())
	    			{
		    			this.currentShape.setShape(cap.resolve().get().getShape());
			...
			y += 20;
			addRenderableWidget(toToolMain = new ShapeToolButton(0, 0, 200, 20, new TranslatableComponent("fecore.shapesgui.mainhand.to"), this, player, InteractionHand.MAIN_HAND, () -> FECoreNetwork.INSTANCE.sendToServer(new ShapeToolSetPacket(InteractionHand.MAIN_HAND, this.currentShape.shape))));
			addRenderableWidget(toToolOffhand = new ShapeToolButton(200, 0, 200, 20, new TranslatableComponent("fecore.shapesgui.offhand.to"), this, player, InteractionHand.OFF_HAND, () -> FECoreNetwork.INSTANCE.sendToServer(new ShapeToolSetPacket(InteractionHand.OFF_HAND, this.currentShape.shape))));
